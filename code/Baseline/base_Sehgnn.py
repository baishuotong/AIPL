...
class MAGNN_metapath_specific(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if self.rnn_type == 'gru':
            ...
            final_r_vec[-1, :, 0] = 1
            for i in range(final_r_vec.shape[0] - 3, -1, -1):
                # consider None edge (symmetric relation)
...
class MAGNN_ctr_ntype_specific(nn.Module):
    def __init__(self,
        ...
        self.Q_fc = nn.Linear(self.out_dim, self.out_dim)
        self.K_fc = nn.Linear(self.out_dim, self.out_dim)
        self.V_fc = nn.Linear(self.out_dim, self.out_dim)
        self.beta = torch.nn.Parameter(torch.ones(1))
        # self.fc2 = nn.Linear(self.out_dim, 1, bias=False)
        # weight initialization
        nn.init.xavier_normal_(self.Q_fc.weight, gain=1.414)
        nn.init.xavier_normal_(self.K_fc.weight, gain=1.414)
        nn.init.xavier_normal_(self.V_fc.weight, gain=1.414)
    ...
    def forward(self, inputs):
        if self.use_minibatch:
            ...
            # metapath-specific layers
            metapath_outs = [F.elu(metapath_layer((g, features, type_mask, edge_metapath_indices, target_idx)).view(-1, self.out_dim))
            ...
            # metapath-specific layers
            metapath_outs = [F.elu(metapath_layer((g, features, type_mask, edge_metapath_indices)).view(-1, self.out_dim))
        ...
        for metapath_out in metapath_outs:
            ...
            Q = self.Q_fc(metapath_out)
            K = self.K_fc(metapath_out)
            V = self.V_fc(metapath_out)
            attn = Q @ (K.transpose(0, 1))

            attn = torch.softmax(attn, dim=-1)
            attn_out= self.beta * (attn @ metapath_out) + metapath_out
            beta.append(attn.view(-1,1024))

        beta = torch.cat(beta, dim=0)
        # beta = F.softmax(beta, dim=0)
        beta = torch.unsqueeze(beta, dim=-1)
        beta.view(7,-1,1024)
        h = torch.sum(beta, dim=0).view(-1,1024)
        return h
